...
def get_keypoint_indices(
    ...
    elif skeleton_format == "soma23":
        mapping = KEYPOINT_MAPPING_SOMA23
    else:
        raise ValueError(
            f"Unsupported skeleton format: {skeleton_format}. "
            "Must be 'rigv1', 'smpl', or 'soma23'"
        )
...
def extract_keypoints_from_motion_smpl_skel(
    ...
    kinematic_info=None,
    keypoint_mapping: Dict[str, str] = KEYPOINT_MAPPING_SMPL,
    ...
    if contacts is not None and kinematic_info is not None:
        ...
        body_name_left_ankle = keypoint_mapping["left_ankle"]
        body_name_right_ankle = keypoint_mapping["right_ankle"]
        body_name_left_toebase = keypoint_mapping["left_foot"]
        body_name_right_toebase = keypoint_mapping["right_foot"]
...
def extract_keypoints_from_motion_soma23_skel(
    all_body_positions: torch.Tensor,
    all_body_rotations_quat: torch.Tensor,
    keypoint_indices_in_mjcf: List[int],
    conceptual_keypoint_names: List[str],
    ...
    """Extract native SOMA23 landmarks without applying SMPL-specific surgery."""
    keypoint_positions = all_body_positions[:, keypoint_indices_in_mjcf, :].clone()
    keypoint_orientations = quaternion_to_matrix(
        all_body_rotations_quat[:, keypoint_indices_in_mjcf, :], w_last=True
    )

    if aux_points:
        left_elbow = conceptual_keypoint_names.index("left_elbow")
        right_elbow = conceptual_keypoint_names.index("right_elbow")
        left_wrist = conceptual_keypoint_names.index("left_wrist")
        right_wrist = conceptual_keypoint_names.index("right_wrist")
        pelvis = conceptual_keypoint_names.index("pelvis")

        # Extend the forearm direction beyond each wrist. This does not depend
        # on SOMA's local joint-axis convention and leaves all 15 native points
        # completely unchanged.
        left_direction = keypoint_positions[:, left_wrist] - keypoint_positions[:, left_elbow]
        right_direction = keypoint_positions[:, right_wrist] - keypoint_positions[:, right_elbow]
        left_direction = left_direction / torch.linalg.norm(
            left_direction, dim=-1, keepdim=True
        ).clamp_min(1.0e-8)
        right_direction = right_direction / torch.linalg.norm(
            right_direction, dim=-1, keepdim=True
        ).clamp_min(1.0e-8)
        left_hand_aux = keypoint_positions[:, left_wrist] + left_direction * 0.20
        right_hand_aux = keypoint_positions[:, right_wrist] + right_direction * 0.20

        pelvis_aux = keypoint_positions[:, pelvis] + torch.einsum(
            "...ij,j->...i",
            keypoint_orientations[:, pelvis],
            torch.tensor(
                [0.16, 0.0, 0.0],
                device=device,
                dtype=keypoint_positions.dtype,
            ),
        )
        keypoint_positions = torch.cat(
            [
        ...
        keypoint_orientations = torch.cat(
            [
                keypoint_orientations,
                keypoint_orientations[:, left_wrist : left_wrist + 1],
                keypoint_orientations[:, right_wrist : right_wrist + 1],
                keypoint_orientations[:, pelvis : pelvis + 1],
            ],
            dim=1,
        )
    ...
    if contacts is not None and kinematic_info is not None:
        left_indices = [
            kinematic_info.body_names.index(KEYPOINT_MAPPING_SOMA23["left_ankle"]),
            kinematic_info.body_names.index(KEYPOINT_MAPPING_SOMA23["left_foot"]),
        ]
        right_indices = [
            kinematic_info.body_names.index(KEYPOINT_MAPPING_SOMA23["right_ankle"]),
            kinematic_info.body_names.index(KEYPOINT_MAPPING_SOMA23["right_foot"]),
        ]
        result["left_foot_contacts"] = (
            contacts[:, left_indices].to(torch.int64).cpu().numpy()
        )
        result["right_foot_contacts"] = (
            contacts[:, right_indices].to(torch.int64).cpu().numpy()
        )
    return result
...
def extract_keypoints_from_motion(
    ...
    if skeleton_format == "rigv1":
        ...
        return extract_keypoints_from_motion_smpl_skel(
            ...
            contacts=contacts,
            kinematic_info=kinematic_info,
            keypoint_mapping=KEYPOINT_MAPPING_SMPL,
        )
    elif skeleton_format == "soma23":
        return extract_keypoints_from_motion_soma23_skel(
            all_body_positions=all_body_positions,
            all_body_rotations_quat=all_body_rotations_quat,
            keypoint_indices_in_mjcf=keypoint_indices_in_mjcf,
            conceptual_keypoint_names=conceptual_keypoint_names,
...
def get_mjcf_path(skeleton_format: str) -> str:
    ...
    if skeleton_format == "rigv1":
        ...
        return "protomotions/data/assets/mjcf/soma23_humanoid.xml"
    else:
        raise ValueError(
            f"Unsupported skeleton format: {skeleton_format}. "
            "Must be 'rigv1', 'smpl', or 'soma23'"
