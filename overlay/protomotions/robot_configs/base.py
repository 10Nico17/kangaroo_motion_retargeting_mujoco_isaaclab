...
class RobotConfig:
    ...
    # Ordered subset of non-root DOFs controlled by the policy.  ``None`` keeps
    # the historical behavior where every DOF is actuated.  Passive joints are
    # still part of kinematic_info, observations, reset states, and motion data.
    ...
    actuated_dof_indices: List[int] = field(init=False)
    ...
    def __post_init__(self):
        ...
        if self.actuated_dof_names is None:
            self.actuated_dof_names = list(self.kinematic_info.dof_names)
        else:
            self.actuated_dof_names = list(self.actuated_dof_names)

        if len(set(self.actuated_dof_names)) != len(self.actuated_dof_names):
            raise ValueError("actuated_dof_names contains duplicate joint names")

        unknown_actuated_dofs = [
            name
            for name in self.actuated_dof_names
            if name not in self.kinematic_info.dof_names
        ]
        if unknown_actuated_dofs:
            raise ValueError(
                "actuated_dof_names contains joints not present in the robot: "
                f"{unknown_actuated_dofs}"
            )

        self.actuated_dof_indices = [
            self.kinematic_info.dof_names.index(name)
            for name in self.actuated_dof_names
        ]
        self.number_of_actions = len(self.actuated_dof_names)
