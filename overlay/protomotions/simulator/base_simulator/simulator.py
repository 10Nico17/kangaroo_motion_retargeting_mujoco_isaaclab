...
class Simulator(RecordingMixin, ABC):
    ...
    def __init__(
        ...
        self._actuated_dof_names: List[str] = self.robot_config.actuated_dof_names
        self._actuated_dof_indices = torch.tensor(
            self.robot_config.actuated_dof_indices,
            dtype=torch.long,
            device=self.device,
        )
    ...
    def _apply_control(self) -> None:
        ...
        if self.control_type == ControlType.BUILT_IN_PD:
            ...
            common_targets = torch.zeros(
                self.num_envs,
                self._num_dof,
                dtype=targets.dtype,
                device=self.device,
            )
            common_targets[:, self._actuated_dof_indices] = targets
            sim_targets = common_targets[:, self.data_conversion.dof_convert_to_sim]
            ...
            actuated_dof_pos = common_dof_state.dof_pos[
                :, self._actuated_dof_indices
            ]
            actuated_dof_vel = common_dof_state.dof_vel[
                :, self._actuated_dof_indices
            ]
            actuated_torques = (
                self._common_p_gains * (targets - actuated_dof_pos)
                - self._common_d_gains * actuated_dof_vel
            )
            actuated_torques = torch.clip(
                actuated_torques,
            ...
            torques = torch.zeros(
                self.num_envs,
                self._num_dof,
                dtype=actuated_torques.dtype,
                device=self.device,
            )
            torques[:, self._actuated_dof_indices] = actuated_torques
    ...
    def _process_control_properties(self) -> None:
        ...
        for dof_idx, dof_name in enumerate(self.robot_config.actuated_dof_names):
    ...
    def _process_action_noise_domain_randomization(
        ...
        dof_indices = get_matching_indices(
            self.robot_config.actuated_dof_names,
