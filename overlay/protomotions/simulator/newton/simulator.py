...
class NewtonSimulator(Simulator):
    ...
    def _create_envs(self) -> None:
        ...
        # Newton's generalized state contains every imported MJCF DOF, not
        # only the policy-controlled subset. Kangaroo has 32 simulated DOFs
        # and 28 actions because its four closed-chain joints are passive.
    ...
    def _set_builder_dof_properties(self, dof_idx: int, info) -> None:
        ...
        has_pd_gains = info.stiffness is not None and info.damping is not None
        if self.control_type == ControlType.BUILT_IN_PD and has_pd_gains:
            ...
            # PROPORTIONAL / TORQUE apply forces themselves. A joint without
            # gains is passive and must also remain uncontrolled in built-in
            # PD mode.
    ...
    def _setup_explicit_pd_arrays(self) -> None:
        ...
        # Explicit kernels index Newton's full generalized state. Policy
        # actions are expanded to this full simulator ordering by the base
        # simulator before reaching this backend.
        num_dofs = self._num_dof
        ...
        for dof_name in self.robot_view.joint_names:
            ...
            for common_dof_name in common_dof_names:
                info = self.robot_config.control.control_info[common_dof_name]
                is_actuated = common_dof_name in self._actuated_dof_names
                kp_list.append(
                    info.stiffness
                    if is_actuated and info.stiffness is not None
                    else 0.0
                )
                kd_list.append(
                    info.damping
                    if is_actuated and info.damping is not None
                    else 0.0
                )
                limit = info.effort_limit
                torque_limits_list.append(
                    limit if is_actuated and limit is not None else 0.0
                )

        if len(kp_list) != num_dofs:
            raise RuntimeError(
                "Newton DOF/control mapping mismatch: "
                f"mapped {len(kp_list)} DOFs, expected {num_dofs}"
            )
    ...
    def _physics_step(self) -> None:
        ...
        if self.control_type == ControlType.BUILT_IN_PD:
            ...
            sim_targets = self._expand_actuated_values_to_sim_dofs(pd_tar)
            ...
            sim_torques = self._expand_actuated_values_to_sim_dofs(torques)
    ...
    def _expand_actuated_values_to_sim_dofs(
        self, actuated_values: torch.Tensor
    ) -> torch.Tensor:
        """Expand policy-sized values to Newton's complete DOF ordering."""
        if actuated_values.shape[-1] != self.robot_config.number_of_actions:
            raise ValueError(
                "Expected actuated values with final dimension "
                f"{self.robot_config.number_of_actions}, got "
                f"{actuated_values.shape[-1]}"
            )
        common_values = torch.zeros(
            self.num_envs,
            self._num_dof,
            dtype=actuated_values.dtype,
            device=self.device,
        )
        common_values[:, self._actuated_dof_indices] = actuated_values
        return common_values[:, self.data_conversion.dof_convert_to_sim]
    ...
    def _init_camera(self) -> None:
        ...
        camera_offsets = getattr(
            self,
            "_fixed_camera_offsets",
            ([0.0, -5.0, 1.0], [0.0, 0.0, 0.2]),
        )
        cam_pos = char_root_pos + np.asarray(camera_offsets[0], dtype=np.float64)
        camera_target = char_root_pos + np.asarray(
            camera_offsets[1], dtype=np.float64
        )
